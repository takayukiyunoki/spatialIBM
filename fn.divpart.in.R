...
# Function to calculating the diversity statics of initial conditions
fn.divpart.in <- function(pool.t0, pool) {
overlap <- fn.dist.HA(pool.t0)
pool$species <- pool.t0$species
LC <- dcast(pool, locations~species, length) [,-1]
LC <- as.data.frame(t(LC))
MC <- MetaCommunity(Abundances = LC, Weights = rep(1, ncol(LC)))
dp.neutral <- DivProfile(seq(0, 2, 1), MC)
dp.functional <- DivProfile(seq(0, 2, 1), MC, Z = overlap)
divpart.in <- data.frame(q.order=c(0,1,2), alpha.neutral=dp.neutral$TotalAlphaDiversity, beta.neutral=dp.neutral$TotalBetaDiversity, gamma.neutral=dp.neutral$GammaDiversity, alpha.functional=dp.functional$TotalAlphaDiversity, beta.functional=dp.functional$TotalBetaDiversity, gamma.functional=dp.functional$GammaDiversity)
return(divpart.in)
}
